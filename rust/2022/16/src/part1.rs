...
pub fn solve(valves: &HashMap<String, Valve>) -> u32 {
    let mut state = State {
        valves,
        minutes: 0,
        valve_name: &String::from("AA"),
        remaining: valves.keys().collect(),
        history: Vec::new(),
        pressure_released: 0,
        max_released: 0,
        loops: 0,
...
fn traverse(mut state: &mut State) -> u32 {
    ...
    let valve: &Valve = state.valves.get(state.valve_name).unwrap();
...
fn highest_estimate(state: &State) -> u32 {
    let remaining_valves: Vec<&Valve> = state.valves
        .iter()
        .filter_map(|(_, valve)| {
            let keep: bool = state.remaining.contains(&valve.name)
                && valve.flow_rate > 0;
